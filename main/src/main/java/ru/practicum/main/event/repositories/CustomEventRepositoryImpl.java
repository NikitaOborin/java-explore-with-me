...
import ru.practicum.main.event.util.EventUtil;
...
public class CustomEventRepositoryImpl implements CustomEventRepository {
    ...
    public List<EventShort> findByParams(EventFilterDto filter) {
        ...
        predicates.add(cb.equal(event.get(EventUtil.PROP_STATE), State.PUBLISHED));

        if (checkForNullOrEmpty(filter.getPaid())) {
            predicates.add(cb.equal(event.get(EventUtil.PROP_PAID), filter.getPaid()));
        }

        if (filter.isOnlyAvailable()) {
            predicates.add(cb.equal(event.get(EventUtil.PROP_LIMIT), filter.isOnlyAvailable()));
        }

        if (checkForNullOrEmpty(filter.getCategories())) {
            CriteriaBuilder.In<Long> inCategories = cb.in(event.get(EventUtil.PROP_CATEGORY));
        ...
        if (checkForNullOrEmpty(filter.getRangeStart()) && checkForNullOrEmpty(filter.getRangeEnd())) {
            predicates.add(cb.between(event.get(EventUtil.PROP_DATE), start, filter.getRangeEnd()));
        } else {
            predicates.add(cb.greaterThanOrEqualTo(event.get(EventUtil.PROP_DATE), start));
        }

        if (checkForNullOrEmpty(filter.getText())) {
            Predicate likeTitle = cb.like(cb.lower(event.get(EventUtil.PROP_ANNOTATION)), "%" + filter.getText().toLowerCase() + "%");
            Predicate likeDescription = cb.like(cb.lower(event.get(EventUtil.PROP_DESCRIPTION)), "%" + filter.getText().toLowerCase() + "%");
            predicates.add(cb.or(likeTitle, likeDescription));
        }
        cq.multiselect(event.get(EventUtil.PROP_ID), event.get(EventUtil.PROP_TITLE), event.get(EventUtil.PROP_ANNOTATION),
                        event.get(EventUtil.PROP_CATEGORY), event.get(EventUtil.PROP_PAID), event.get(EventUtil.PROP_DATE),
                        event.get(EventUtil.PROP_INITIATOR), event.get(EventUtil.PROP_PUBLISHED))
                .groupBy(event.get("id"));

        if (checkForNullOrEmpty(filter.getSort())) {
            if (filter.getSort().equals(StateSort.EVENT_DATE)) {
                cq.orderBy(cb.asc(event.get(EventUtil.PROP_DATE)));
            }
        }
    ...
    public List<Event> findByParamAdmin(EventFilterAdminDto filter) {
        ...
        if (checkForNullOrEmpty(filter.getUsers())) {
            CriteriaBuilder.In<Long> inUsers = cb.in(event.get(EventUtil.PROP_INITIATOR));
            filter.getUsers().forEach(inUsers::value);
            predicates.add(inUsers);
        }

        if (checkForNullOrEmpty(filter.getStates())) {
            CriteriaBuilder.In<State> inStates = cb.in(event.get(EventUtil.PROP_STATE));
